...
*)
    ...
    *)
    ...
    let ``33 Partially specifying arguments (Part 2).`` () =
        ...
        let f animal noise = animal + " says " + noise
        let howl k = (k|>f) "slash/crunch/snap" // <- multiple words on this line.  You MUST use `f`.
    ...
    let ``34 Partially specifying arguments (Part 3).`` () =
        ...
        let f animal noise = animal + " says " + noise
        let  x=  ", de gozaru"
        let cows b = (f "cow" )<|x // <-- multiple words on this line, or you may want to make this a multi-line thing.  You MUST use `f`.
        cows "moooo"|> should equal "cow says moooo"
